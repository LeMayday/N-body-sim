...
public class Celestial_Bodies {
	...
	public Celestial_Bodies(Simulation s, int[] scales) {
		SDS = scales[0];
		SMS = scales[1];
		STS = scales[2];
	...
	public void push(double[] params) {
		push(Q1, params[0]);
		push(Q2, params[1]);
		push(masses, params[4]);
	...
	private void computeForcesOnBody(int index) {
		...
		for (int j = 0; j < size; j++) {
			if (index != j) {
				double distQ1 = Q1[j] - Q1[index];
				double distQ2 = Q2[j] = Q2[index];
				double r = Math.sqrt(distQ1*distQ1 + distQ2*distQ2);
				
				double gradUq1 = G * masses[j] * masses[index] / (r * r * r) * distQ1;
				double gradUq2 = G * masses[j] * masses[index] / (r * r * r) * distQ2;
	...
	public void incrementPositions(KeyEvent e, double incrementFactor){
		for(int i = 0; i < size; i++) {
			switch(e.getKeyCode()){
			case KeyEvent.VK_W:
				Q2[i] += incrementFactor;
				break;
			case KeyEvent.VK_S:
				Q2[i] -= incrementFactor;
				break;
			case KeyEvent.VK_D:
				Q1[i] -= incrementFactor;
				break;
			case KeyEvent.VK_A:
				Q1[i] += incrementFactor;
				break;
